...
class ScreeningPartThreeFormValidator(FormValidator):
    def clean(self):
        ...
        self.required_if(YES, field="hba1c_performed",
                         field_required="hba1c")
        ...
        self.required_if_true(self.cleaned_data.get("fasting_glucose"),
                              field_required="fasting_glucose_datetime")

        self.required_if_true(self.cleaned_data.get("ogtt_two_hr"),
                              field_required="ogtt_two_hr_units")

        self.required_if_true(self.cleaned_data.get("ogtt_two_hr"),
                              field_required="ogtt_two_hr_datetime")

        self.not_required_if(NO, field="fasted",
                             field_not_required="ogtt_base_datetime",
                             inverse=False)
        self.not_required_if(NO, field="fasted",
                             field_not_required="ogtt_two_hr_datetime",
                             inverse=False)
        self.not_required_if(NO, field="fasted",
                             field_not_required="ogtt_two_hr",
                             inverse=False)
        self.not_required_if(NO, field="fasted",
                             field_not_required="ogtt_two_hr_units",
                             inverse=False)
    ...
    def validate_ogtt_dates(self):
        if (self.cleaned_data.get("ogtt_base_datetime")
                and self.cleaned_data.get("ogtt_two_hr_datetime")):
            ...
            tdelta = (self.cleaned_data.get("ogtt_two_hr_datetime")
    ...
    def validate_creatinine(self):
        ...
        self.required_if_true(self.cleaned_data.get("creatinine"),
                              field_required="creatinine_units")

    def validate_glucose_dates(self):
        fasting_glucose_dte = self.cleaned_data.get(
            "fasting_glucose_datetime")
        ogtt_performed_dte = self.cleaned_data.get(
            "ogtt_base_datetime")
        ogtt_two_hr_dte = self.cleaned_data.get(
            "ogtt_two_hr_datetime")
        if fasting_glucose_dte and ogtt_performed_dte:
            ...
            diff = (ogtt_two_hr_dte -
                    ogtt_performed_dte).total_seconds() / 60.0
            if diff <= 1.0:
                raise forms.ValidationError({
                    "ogtt_two_hr_datetime": (
